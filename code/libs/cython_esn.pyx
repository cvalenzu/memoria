...
cpdef _collect_states(X,activation,Win, W,leaking_rate, initLen, n_reservoir,n_inputs):
  N = len(X)
  ...
  for t in range(N):
      ...
      last_state = (1-leaking_rate)*last_state  + leaking_rate*activation( np.dot( Win, np.hstack((1,u)) ) \
                                                            + np.dot( W, last_state  ) )
      if t >= initLen:
        X_states[t-initLen,:] = np.hstack((1,u,last_state ))
  return X_states, last_state
