...
for i,result_args in results_df.iterrows():
    ...
    if result_args.preproc == "minmax":
        ...
        standarization_out.fit(y_train.reshape(-1,1))
    ...
    y_train = preproc_out.transform(y_train.reshape(-1,1)).reshape(-1) if preproc_out else y_train
    ...
    score_train = metrics.mean_squared_error(y_train_multi,y_pred)
    ...
    score_test = metrics.mean_squared_error(y_test_multi,y_pred_test)
    ...
    np.savetxt(base_results+"esn_y_train_{}_one.csv".format(source),y_train_orig)
    np.savetxt(base_results+"esn_y_test_{}_one.csv".format(source),y_test)

    np.savetxt(base_results+"esn_y_approx_train_{}lags_{}_one.csv".format(result_args.lags, source),y_pred)
    np.savetxt(base_results+"esn_y_approx_test_{}lags_{}_one.csv".format(result_args.lags, source),y_pred_test)

best_scores = pd.DataFrame(best_scores)
best_scores.to_csv(base_results+"esn_{}_best_scores_one.csv".format(source))
