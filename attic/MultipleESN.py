...
for i in range(prediction_steps):
    ...
    y_pred[:,i] = clf.predict(X_test).reshape((1,-1))
    best_regressor.append(clf)
print("Saving CV results")
pickle.dump(regressors, open("regressors_{}.p".format(args.output), "wb"))

print("Saving Best Data")
pickle.dump(best_regressor, open("best_esn_{}.p".format(args.output),"wb"))
